...
func TestTrafficLight(t *testing.T) {
	...
	m.Action(">red", func(Event) {
		i += 1
	})
	m.Action(">green", func(Event) {
		i += 2
	})
	m.Action(">yellow", func(Event) {
		i += 3
	})
	m.Send("signal")
	if i != 3 {
...
func TestCancel(t *testing.T) {
	...
	m := NewMachine("A")
	m.Rule("signal", "A", "B")
	m.Action(">B", func(Event) {
		i += 1
	})
	m.Action("<A", func(Event) {
		m.Cancel()
		j += 1
	})
	m.Send("signal")
...
func TestNestedSends(t *testing.T) {
	...
	m.Action(">C", func(Event) {
		m.Send("signal")
		i += 2
	})
	m.Action(">D", func(Event) {
		i += 3
	})
	m.Send("signal")
	if i != 6 {
...
func TestFluent(t *testing.T) {
	m := NewMachine("A")
	m.InState("A").On("signal", "B")
	m.Send("signal")
	if m.currentState != "B" {
		t.Error("Fluent rule failed ")
	}
}

func TestFluentWithAction(t *testing.T) {
	i := 0
	m := NewMachine("A")
	m.InState("A").On("signal", "B").WithAction(func(Event) {
		i += 1
	})
	m.Send("signal")
	if i != 1 {
		t.Error("Fluent rule action failed.")
	}
}
