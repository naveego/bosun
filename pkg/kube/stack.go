...
func NewStack(cluster *Cluster, name string) (*Stack, error) {
	...
	for _, candidate := range cluster.StackTemplates {
		...
		if match, _ := regexp.MatchString(candidate.NamePattern, name); match {
...
func (k *Stack) GetNamespace(role core.NamespaceRole) (NamespaceConfig, error) {
	...
	return NamespaceConfig{}, errors.Errorf("kubernetes cluster config %q does not have a namespace for the role %q", k.StackTemplate.Namespaces, role)
}
